...
public class Ban extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        ProxyBan.getProxyBan().getProxy().getScheduler().runAsync(ProxyBan.getProxyBan(), () -> {
            String banner;
            if(sender instanceof ProxiedPlayer) banner = sender.getName(); else banner = "console";
            if(args.length == 0 ) {
                sender.sendMessage(new TextComponent("§9§lUżycie: /proxyban §f§l<nick> §f§l<data> §f§l<powod> §9§llub /proxyban §f§l<nick> §f§l<powód>"));
                return;
            }
    ...
    private void kickPlayers(String playerNick){
        if(ProxyBan.getProxyBan().getProxy().getPlayer(playerNick) != null) {
            String ip = ProxyBan.getProxyBan().getProxy().getPlayer(playerNick).getPendingConnection().getAddress().getAddress().getHostAddress();
